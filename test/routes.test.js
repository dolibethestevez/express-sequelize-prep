...
describe("Tweet Routes", function() {
    ...
    it("returns tweets if there are any in the database", function() {
      return Tweet.create({
        ...
        return agent
          ...
          .expect(function(res) {
            expect(res.body).to.be.an.instanceOf(Array);
            expect(res.body).to.include.a.thing.with.property(
              "text",
              tweet.text
            );
          });
      ...
      return Promise.all([
        ...
        return agent
          ...
          .expect(function(res) {
            ...
            expect(res.body).to.not.include.a.thing.with.property(
              "text",
              "Why is nobody talking about #sequelize #whatsthedeal"
            );
          });
      });
    });
  ...
  describe("POST /tweets", function() {
    ...
    it("saves the article to the DB", function() {
      return Tweet.findOne({ where: { text: "Awesome new tweet" } }).then(
        function(article) {
  ...
  describe("PUT /tweets/:id", function() {
    ...
    it("saves the update", function() {
      return Tweet.findById(tweet.id).then(function(tweet) {
